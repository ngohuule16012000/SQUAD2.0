...
app.add_url_rule('/history', 'history', hischatbox)
...
def chatJson(user, bot, f1_score):
   return {"user_chat": user, "bot_chat": bot, "f1_score": f1_score, "timestamp": str(datetime.datetime.now())}
...
def prediction_message():
   ...
   if predict == "":
      predict = "Xin lỗi không có câu trả lời cho câu hỏi này!"
   chat_history.append(chatJson(message, predict, f1_scores))
